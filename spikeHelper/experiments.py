...
def expAllRats(train_size=.9):
    ...
    rat = 'rat 7'
    results = experiment1rat(data,rat,results,train_size)
...
def experiment1rat(data,rat,results,train_size,n_splits=[10, 30]):
    ...
    directResults = bothSame(grid.best_estimator_, data, n_splits = n_splits[0],train_size=train_size)
    ...
    crossResults = bothCross(grid.best_estimator_, data, n_splits=n_splits[1],train_size=train_size)
    ...
    corrEvo, kappaEvo = expEvolution(grid.best_estimator_, data, n_splits=n_splits[1],train_size=train_size)
...
def expEvolution(clf, data, n_splits,train_size):
    predictions = eachCross(clf, data[data['end'] ], data, n_splits,train_size)
    evolutionRes = {}
...
def bothSame(clf, data, n_splits,train_size):
    results = {}

    ypred, ytrue = eachSame(clf, data[data['end']], n_splits,train_size)
    results['late'] = {}
    ...
    ypred, ytrue = eachSame(clf, data[data['beg']], n_splits,train_size)
    results['early'] = {}
...
def eachSame(clf, data, n_splits,train_size):
    sh = StratifiedShuffleSplit(n_splits = n_splits, test_size=None, train_size = train_size)
    trial = data['trial']
    ypred = [];
...
def bothCross(clf, data, n_splits,train_size):
    crossResults = {}

    train = data[data['beg']]; test = data[data['end']];
    trainBegTestEnd = eachCross(clf, train, test, n_splits,train_size);
    ...
    trainEndTestBeg = eachCross(clf, train, test, n_splits,train_size);
...
def eachCross(clf, train, test, n_splits,train_size):
    sh = StratifiedShuffleSplit(n_splits = n_splits,test_size=None, train_size = train_size)
